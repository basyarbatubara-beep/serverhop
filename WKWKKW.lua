...
AutoHopBtn.Text = "Auto Hop to Solo Server"
...
updateActiveTab = function(newTab)
    ...
    if not isHopping then
        ...
        elseif activeTab == "small" then
            AutoHopBtn.Text = "Auto Hop to Small Server"
...
local function startHopSequence()
    ...
    for _, srv in ipairs(loadedServers) do
        ...
        local passMax = (not maxLimit) or (p <= maxLimit)
        if srv.id ~= CurrentJobId and not blacklistServers[srv.id] and passTab and passMax and p < (srv.maxPlayers or 999) then
            table.insert(hopCandidates, srv)
        end
    end

    -- Strict sort: lowest players first, lowest latency first
